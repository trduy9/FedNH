    ...
    def aggregate(self, client_uploads, round):
        ...
        with torch.no_grad():
            for idx, local_prototypes in enumerate(client_uploads):
                dir_round = f"prototype_round/round_{round}"
                if not osp.exists(dir_round):
                    os.makedirs(dir_round)
                torch.save(client_uploads, f"prototype_round/round_{round}/prototype_client_{idx}.pt")
            ...
            torch.save(updated_proptos, f"prototype_round/round_{round}/avg_server_proto{round}.pt")
